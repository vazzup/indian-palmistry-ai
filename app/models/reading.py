"""
Reading model for palm readings.
"""
...
class ReadingStatus(enum.Enum):
    """Status enum for reading processing."""
    QUEUED = "queued"
...
class Reading(Base):
    ...
    # Job tracking
    status = Column(Enum(ReadingStatus), default=ReadingStatus.QUEUED, nullable=False, index=True)
    ...
    conversations = relationship("Conversation", back_populates="reading", cascade="all, delete-orphan")
    
    def __repr__(self):
        return f"<Reading(id={self.id}, user_id={self.user_id}, status={self.status.value})>"